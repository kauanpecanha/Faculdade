...
\newcommand{\repositoriocodigos}{https://github.com/kauanpecanha/Faculdade/tree/master/MEDO2/trabalho1/questions}
\newcommand{\linkcodigo}[2]{%
    \par\noindent\textbf{Arquivo de código utilizado nesta solução: }
    \href{#1}{\nolinkurl{#2}}.\par
}
...
\addcontentsline{toc}{chapter}{CÓDIGOS-FONTE}

Os códigos utilizados nas soluções deste trabalho estão disponíveis no
repositório:

\begin{center}
    \url{\repositoriocodigos}
\end{center}
...
\linkcodigo
{https://github.com/kauanpecanha/Faculdade/blob/master/MEDO2/trabalho1/questions/l1-question1/index.py}
{questions/l1-question1/index.py}
...
\linkcodigo
{https://github.com/kauanpecanha/Faculdade/blob/master/MEDO2/trabalho1/questions/l3-question1/index.py}
{questions/l3-question1/index.py}

\subsection{Solução analítica}

Pelo método das características, toma-se uma curva $x=x(t)$ tal que
...
Ao longo dessa característica, a EDP fornece
...
Integrando e usando $u(x_0,0)=x_0^2$, obtém-se
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\linewidth]{01_solucao_edp.png}
    \caption{Perfis de $u(x,t)$ em diferentes instantes}
    \label{fig:lista3-q1-perfis-objetiva}

    {\small Fonte: elaborado pelo autor.\par}
\end{figure}

Os gráficos são parábolas de mesma abertura. Em cada instante $t$, o vértice
está em

\[
    (x_{\mathrm v},u_{\mathrm v})=(3t,t).
\]

Assim, o perfil inicial se desloca para a direita e, simultaneamente, é
elevado pela fonte.

\subsection{Análise física}

O termo $3u_x$ transporta a onda para a direita com velocidade constante
$3$, enquanto o termo-fonte unitário aumenta todos os valores de $u$ à taxa
de uma unidade por unidade de tempo. Como as características
$x=x_0+3t$ são retas paralelas, a forma da parábola é preservada e não há
cruzamento de características nem formação de choques.
...
\linkcodigo
{https://github.com/kauanpecanha/Faculdade/blob/master/MEDO2/trabalho1/questions/l3-question2/index.py}
{questions/l3-question2/index.py}

\subsection{Solução por características}

Seja $X(t;x_0)$ a característica que parte de $x_0$, isto é,

\[
    X(0;x_0)=x_0,
    \qquad
    \frac{dX}{dt}=a(X(t;x_0),t).
\]
...
Logo, $u$ é constante em cada característica:

\[
    u(X(t;x_0),t)=u_0(x_0).
\]

Se o mapa $x_0\mapsto X(t;x_0)$ for invertível, o ponto inicial da
característica que chega a $(x,t)$ é $x_0=X^{-1}(x,t)$. Portanto,

\[
    \boxed{u(x,t)=u_0\!\left(X^{-1}(x,t)\right)}.
\]

No caso particular $a(x,t)=c$, as características são
...
Como o enunciado não especifica $a(x,t)$ nem $u_0(x)$, não existe um único
gráfico para o caso geral. Para representar o transporte, considera-se o
exemplo $c=2$ e $u_0(x)=e^{-x^2}$, para o qual
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\linewidth]{03_solucao_questao2.png}
    \caption{Perfis de $u(x,t)=e^{-(x-2t)^2}$ em diferentes instantes}
    \label{fig:lista3-q2-perfis-objetiva}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.78\linewidth]{04_caracteristicas_questao2.png}
    \caption{Características $x=x_0+2t$ do exemplo}
    \label{fig:lista3-q2-caracteristicas-objetiva}
...
A equação descreve o transporte do perfil inicial sem fonte e sem difusão:
o valor de $u$ permanece constante ao longo de cada característica. Para
velocidade constante, o perfil conserva forma e amplitude e apenas se
desloca: para a direita se $c>0$ e para a esquerda se $c<0$. No exemplo,
a onda se move para a direita com velocidade $2$, mantendo amplitude máxima
igual a $1$.

Quando $a$ depende de $x$ ou de $t$, as características podem ser curvas e o
perfil pode sofrer compressão ou expansão. A expressão obtida permanece
válida enquanto o mapa das características for invertível.
...
\begin{enumerate}
    \item Determinar as curvas caracteristicas
    \item Encontre o caminho do choque
    \item Esboce o diagrama no plano $(x, t)$ e a solução $u(x, t)$
\end{enumerate}

\section{Solução}

\linkcodigo
{https://github.com/kauanpecanha/Faculdade/blob/master/MEDO2/trabalho1/questions/l4-question6/index.py}
{questions/l4-question6/index.py}

\subsection{Curvas caracteristicas}

A equação pode ser escrita na forma de uma lei de conservação:
...
Ao longo de uma curva característica $x=x(t)$, tem-se
...
Escolhendo $x'(t)=u$, a equação fornece $du/dt=0$. Portanto, $u$
permanece constante em cada característica e

\[
    x(t)=x_0+u_0(x_0)t.
\]

Para $x_0<0$, o valor transportado é $u=-2$, de modo que

\[
    \boxed{x(t)=x_0-2t,\qquad x_0<0.}
\]

Para $x_0>0$, o valor transportado é $u=0$, logo

\[
    \boxed{x(t)=x_0,\qquad x_0>0.}
\]

As características à esquerda deslocam-se com velocidade $-2$, enquanto as da
direita são verticais no plano $(x,t)$. Assim, elas se afastam da origem,
deixando entre $x=-2t$ e $x=0$ uma região que deve ser preenchida. Esse
afastamento é o mecanismo que produz uma onda de rarefação.

\subsection{Verificação do caminho de choque}

Se os dois estados fossem ligados por uma descontinuidade, a condição de
Rankine--Hugoniot daria a velocidade
...
\subsection{Visualização gráfica da solução}

A solução é autossimilar, ou seja, depende da razão
$\xi=x/t$. Dentro do leque, procura-se $u(x,t)=U(\xi)$. Substituindo essa
forma na equação, obtém-se

\[
    \left(U(\xi)-\xi\right)U'(\xi)=0.
\]

Na parte não constante da solução, $U'(\xi)\neq0$, portanto
$U(\xi)=\xi$. Ligando os estados $u_L=-2$ e $u_R=0$, resulta

\[
    \boxed{
    u(x,t)=
    \begin{cases}
        -2, & \dfrac{x}{t}\leq -2,\\[6pt]
        \dfrac{x}{t}, & -2<\dfrac{x}{t}<0,\\[6pt]
        0, & \dfrac{x}{t}\geq0,
    \end{cases}
    \qquad t>0.}
\]

Equivalentemente, as fronteiras do leque são $x=-2t$ e $x=0$. Entre elas,
a solução cresce linearmente de $-2$ até $0$. O salto inicial não permanece
como choque: ele se abre progressivamente, e a largura da zona de transição é
$2t$.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.78\linewidth]{06_caracteristicas_rarefacao.png}
    \caption{Diagrama de características e leque de rarefação}
    \label{fig:caracteristicas-rarefacao}

    {\small Fonte: elaborado pelo autor.\par}
\end{figure}

Na Figura~\ref{fig:caracteristicas-rarefacao}, as retas verdes que partem da
origem preenchem o setor $-2t\leq x\leq0$. Cada uma delas possui inclinação
$x/t=\xi$ e transporta o valor constante $u=\xi$.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.78\linewidth]{07_perfis_rarefacao.png}
    \caption{Perfis da solução entrópica em diferentes instantes}
    \label{fig:perfis-rarefacao}

    {\small Fonte: elaborado pelo autor.\par}
\end{figure}

Como mostra a Figura~\ref{fig:perfis-rarefacao}, o estado $u=-2$ permanece à
esquerda de $x=-2t$, e o estado $u=0$ permanece à direita de $x=0$. O leque
se alarga com o tempo sem ultrapassar esses dois valores, caracterizando uma
onda de rarefação.
